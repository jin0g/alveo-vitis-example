import numpy as np
import time
from libmv_module_hw import MVRunner

MEGA = 1024 * 1024

def test_mv_hw():
    MATRIX_SIZE = 32
    print(f"Running MV hardware test (via Python) with matrix size: {MATRIX_SIZE}x{MATRIX_SIZE} and vector size: {MATRIX_SIZE}")
    ...
    XCLBIN_FILE = "mv.xclbin"
    try:
        runner = MVRunner(XCLBIN_FILE)
    except Exception as e:
        print(f"Error initializing MVRunner with {XCLBIN_FILE}: {e}")
        print(f"Please ensure '{XCLBIN_FILE}' exists and XRT is set up correctly.")
        return

    print(f"Running MV HW kernel ({XCLBIN_FILE})...")
    num_iterations = 5
    kernel_times = []
    total_times = []

    try:
        result_hw = runner.run(a, x)
    except Exception as e:
        print(f"Error during MVRunner.run (initial): {e}")
        return
